...
def main():
    ...
    changed_files = Git.get_diff_files(
        remote_name=remote_name, head_ref=vars.head_ref, base_ref=vars.base_ref
    )
    ...
    for file in changed_files:
        ...
        if file_extension not in vars.target_extensions:
            Log.print_yellow(
                f"Skipping, unsuported extension {file_extension} file {file}"
            )
            continue
        ...
        Log.print_green(
            f"Asking AI. Content Len:{len(file_content)} Diff Len: {len(file_diffs)}"
        )
        response = ai.ai_request_diffs(code=file_content, diffs=file_diffs)

        log_file.write(
            f"{separator}{file_content}{separator}{file_diffs}{separator}{response}{separator}"
        )
            ...
            for response in responses:
                if response.line:
                    result = post_line_comment(
                        ...
                        text=response.text,
                        line=response.line,
                        commit_sha=vars.head_commit_sha,
                    )
                if not result:
                    result = post_general_comment(
                        github=github, file=file, text=response.text
                    )
                if not result:
